...
@router.get("/is-deployed/", response_model=IsDeployed)
def get_is_deployed(request: Request):
    result = request.app.col_name_plant_is_deployed.find_one()

    if (result is not None):
        return result

    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                        detail=f"Is deployed flag is not found")
...
def extract_json_without_mongoid(last_image_json):
    # Check if the input dictionary has the '_id' key
    if '_id' in last_image_json:
        # Remove the '_id' key
        del last_image_json['_id']

    return last_image_json
...
def create_image_potato(request: Request, image_create: PlantImageCreate):
    ...
    # Use the function to extract the JSON without the '_id' field
    last_image = extract_json_without_mongoid(last_image_json)
    last_image["id"] += 1
